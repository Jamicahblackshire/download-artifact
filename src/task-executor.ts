...
import { AgentConfig, defaultAgentConfig } from '../e2b.config';
import { Task } from './agent-orchestrator';
...
const executeTasks = async (): Promise<void> => {
  ...
  try {
    ...
    const mediumPriorityTask = EXAMPLE_TASKS.find(
      (t) => t.priority === 'medium' && t.service === 'cloudflare'
    );
